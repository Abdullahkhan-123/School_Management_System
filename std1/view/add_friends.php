...
if(mysqli_num_rows($result) > 0){
	while($row=mysqli_fetch_assoc($result)){
		...
		if ($result1) {
			$row1 = mysqli_fetch_assoc($result1); // Fetch the row data

			if ($row1) {
				$status = $row1['_status'];
			} else {
				// Handle the case where no row was found
				// You can set a default value for $status or display an error message
				$status = "Status not found";
			}
		} else {
			// Handle the case where the query failed
			// You can log an error or display an error message
